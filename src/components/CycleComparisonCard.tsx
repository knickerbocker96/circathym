  ...
  if (!personalCycleLength) {
    ...
    return (
      ...
      >
        ...
        {open && (
          <div className="px-5 pb-5 pt-4 space-y-2" style={{ borderTop: '1px solid var(--apple-separator)' }}>
        ...
        <>
          ...
          >
            <div className="flex flex-wrap gap-x-3 gap-y-1 mb-2">
              <EvidencePill label={`${sleepLog.length} nights`} />
              <EvidencePill label={`${goodWakes} quality wakes`} />
              <EvidencePill label={`${confidence.label} confidence`} color={confidence.label === 'High' ? 'var(--apple-green)' : confidence.label === 'Medium' ? 'var(--apple-orange)' : 'var(--muted-foreground)'} />
            </div>
